...
const feedbackCategoryCollection = client
  .db("experiment-labs")
  .collection("feedbackCategories");
...
module.exports.addAFeedbackCategory = async (req, res, next) => {
  ...
  const existingData = await feedbackCategoryCollection.findOne({
    organizationId,
  });
...
module.exports.updateACategoryNameAndRating = async (req, res, next) => {
  ...
  const organization = await feedbackCategoryCollection.findOne({
    organizationId,
  });
  if (!organization) {
...
module.exports.getAFeedbackCategoryByOrganizationId = async (
  req,
  res,
  next
) => {
...
module.exports.addAFeedbackItem = async (req, res, next) => {
  const { organizationId, courseId, categoryName, rating, feedbackItem } =
    req.body;
  ...
  if (existingFeedbackItem) {
    res.status(400).json({
      error:
        "redemption item name already exists. Please provide another name.",
    });
    return;
  }
...
module.exports.updateAFeedbackItem = async (req, res, next) => {
  const { organizationId, courseId, categoryName, oldItemName, feedbackItem } =
    req.body;
        ...
        category.feedbackItems = category.feedbackItems?.map((existingItem) => {
          if (existingItem.feedbackItemName === oldItemName) {
            return {
              ...
              userFormFields: feedbackItem.userFormFields,
              expertMentorFormFields: feedbackItem.expertMentorFormFields,
              executionMentorFormFields: feedbackItem.executionMentorFormFields,
